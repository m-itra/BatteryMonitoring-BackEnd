...
В Infrastructure/.env используются две группы переменных:
...
- Infrastructure/migrations/user-db/env.py
- Infrastructure/migrations/battery-db/env.py
...
1. В Infrastructure/.env хранятся значения:
   - `DOCKER_USER_DATABASE_URL`
   - `D[README.md](../README.md)OCKER_BATTERY_DATABASE_URL`
2. В Infrastructure/docker-compose.ymlони передаются в контейнеры миграций как:
   - `USER_DATABASE_URL`
   - `BATTERY_DATABASE_URL`
...
- [nfrastructure/protos/user_service.proto
