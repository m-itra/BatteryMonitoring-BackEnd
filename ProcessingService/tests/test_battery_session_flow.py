...
def test_interrupt_stale_session_ignores_large_server_gap_when_client_gap_is_small():
    device = Device(
        device_id=uuid4(),
        user_id=uuid4(),
        reference_capacity_mwh=40000,
    ...
    sample = BatterySample(
        boot_session_id=active_session.boot_session_id,
        sample_seq=43,
        client_time=active_session.last_client_time + timedelta(seconds=30),
        ac_connected=False,
        is_charging=False,
        charge_percent=54.8,
        remaining_capacity_mwh=20000,
    ...
    with (
        ...
        next_active_session, completed_cycles = asyncio.run(
            interrupt_stale_session_if_needed(
                ...
                active_session=active_session,
                device=device,
                received_at=datetime.fromisoformat("2026-05-07T10:10:10"),
                sample=sample,
            )
        )

    assert next_active_session is active_session
    assert completed_cycles == 0
    close_mock.assert_not_awaited()
    cycle_mock.assert_not_awaited()


def test_interrupt_stale_session_closes_session_on_sample_sequence_gap():
    device = Device(
        device_id=uuid4(),
        user_id=uuid4(),
        reference_capacity_mwh=40000,
    ...
    with (
        ...
        next_active_session, completed_cycles = asyncio.run(
            interrupt_stale_session_if_needed(
                ...
                active_session=active_session,
                device=device,
                received_at=datetime.fromisoformat("2026-05-07T10:05:35"),
                sample=sample,
            )
        )
...
def test_interrupt_stale_session_drops_small_session_on_sample_sequence_gap():
    device = Device(
        device_id=uuid4(),
        user_id=uuid4(),
        reference_capacity_mwh=40000,
    ...
    active_session = BatteryActiveSession(
        ...
        last_sample_seq=42,
        start_charge_percent=80.0,
        current_charge_percent=79.5,
        discharged_energy_mwh=50.0,
    ...
    sample = BatterySample(
        boot_session_id=active_session.boot_session_id,
        sample_seq=44,
        client_time=active_session.last_client_time + timedelta(seconds=10),
        ac_connected=False,
        is_charging=False,
        charge_percent=79.4,
        remaining_capacity_mwh=20000,
    ...
    with (
        ...
        next_active_session, completed_cycles = asyncio.run(
            interrupt_stale_session_if_needed(
                ...
                active_session=active_session,
                device=device,
                received_at=datetime.fromisoformat("2026-05-07T10:05:15"),
                sample=sample,
            )
        )
